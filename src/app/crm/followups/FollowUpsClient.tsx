...
function FollowUpsInner({
  ...
  initial: { queue: Queue; items: FollowUp[]; counts: Record<string, number> };
}) {
  const [queue, setQueue] = useState<Queue>(initial.queue);
  const [items, setItems] = useState<FollowUp[]>(initial.items);
  ...
  // The server sent the queue this page opened on; only switching tabs fetches.
...
export default function FollowUpsClient({
  ...
  return (
    <Suspense fallback={<div className="panel p-5 text-sm">Loading…</div>}>
      <FollowUpsInner initial={initial} />
    </Suspense>
  );
}
