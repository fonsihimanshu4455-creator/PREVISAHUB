  ...
  const load = useCallback(async () => {
    ...
    const results = await Promise.all(
      TABS.map((t) => {
        ...
        return fetch(`/api/crm/leads?${p}`).then((r) => r.json());
      })
    );
    const next: Record<string, number> = {};
    TABS.forEach((t, i) => (next[t.key] = results[i]?.total ?? 0));
    setCounts(next);
    setLeads(results[TABS.findIndex((t) => t.key === tab)]?.leads ?? []);
    setBusy(false);
  }, [tab]);
