    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button
          ...
          className="rounded-lg bg-accent px-4 py-2.5 text-[13.5px] font-semibold text-white transition hover:brightness-95"
        >
          + Add Staff
        </button>
      </div>

      <div className="panel px-4 py-3 text-sm text-[color:var(--text-muted)]">
        <b>Counsellors</b> sign in at{" "}
        <a href="/staff" target="_blank" className="font-semibold text-accent hover:underline">
          /staff
        </a>
        , <b>telecallers</b> at{" "}
        <a href="/calling" target="_blank" className="font-semibold text-accent hover:underline">
          /calling
        </a>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-line">
              {staff.map((s) => (
                <tr key={s.id} className="hover:bg-accent/[0.04]">
                  <td className="px-4 py-3 font-semibold text-[color:var(--text)]">{s.name}</td>
                  ...
                  <td className="px-4 py-3">
                    <select
                      ...
                      className="rounded-lg border border-line-strong px-2 py-1 text-xs font-semibold outline-none focus:border-accent"
                    >
                    ...
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${
                        s.active
                          ...
                          : "bg-surface-sunk text-[color:var(--text-muted)] ring-slate-200"
                      }`}
                    >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      <button
                        ...
                        className="rounded-lg border border-line-strong px-3 py-1.5 text-[12px] font-semibold text-[color:var(--text-muted)] transition hover:bg-surface-sunk"
                      >
              ...
              {!loading && staff.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center text-sm text-[color:var(--text-faint)]">
...
function ResetPasswordModal({
  ...
  return (
    <Modal title={`Reset password · ${staff.name}`} onClose={onClose}>
      ...
      {msg && <p className="mt-2 text-sm text-[color:var(--text-muted)]">{msg}</p>}
...
function Field({
  ...
  return (
    <label className="block">
      ...
      {children}
      {hint && <span className="mt-1 block text-[11px] text-[color:var(--text-faint)]">{hint}</span>}
    </label>
  );
}
