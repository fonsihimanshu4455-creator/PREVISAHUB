...
// The dashboard's rendering. The data arrives as a prop, already counted on
// the server, so this paints on first render with no spinner and no fetch.
...
import type { StatsPayload } from "@/lib/leads/dashboard";

export default function DashboardView({ data }: { data: StatsPayload }) {
  const { user } = useCrm();
  const k = data.kpis;
